...
$customer_password_err = $confirm_password_err = "";
...
if(isset(($_POST['customer_reset_password_btn']))){
    ...
    if(empty($customer_password_err) && empty($confirm_password_err)){
        
        // Prepare an insert statement
        $sql = "UPDATE customer SET customer_password=:customer_password WHERE userID=:userID";
         
        if($stmt = $pdo->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bindParam(":userID", $userID, PDO::PARAM_STR);
            $stmt->bindParam(":customer_password", $param_customer_password, PDO::PARAM_STR);
            
            // Set parameters
            $param_customer_password = password_hash($customer_password, PASSWORD_DEFAULT); // Creates a password hash
...
<div class="form_container">
    ...
    <form class="sign_form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <div class="form-btn">
                <input type="submit" class="small-btn" name="customer_reset_password_btn" value="Reset">
            
            <p style="color:#fff;">Sign In to Customer Account? <a href="../customer_signIn.php" style="color:#fff;">Sign In here</a>.</p>
                </div>
        </form>
